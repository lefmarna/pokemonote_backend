...
この「メタタグの動的書き換え」がまさに、Nuxt で実現できなかった部分になります。（厳密に言えば、SSR モードの Nuxt であれば可能だが、SPA モードでは不可）
Vue では$route でルーティングを監視することができ、タイトルやメタディスクリプションの書き換えを実現することができたため、これが Vue を後押ししてくれた理由となりました。
